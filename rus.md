...
Вы ведь помните Grunt? Я [достаточно много писал о нем][04] в прошлом, и [даже написал книгу][05],
в которой я предложил использовать его как инстромент для сборки. Это
потрясающий инструмент, вы должны как-нибудь обязательно его попробовать.

[Некоторые люди][06] считают что Gulp намного лучше Grunt, и обещают Gulp
множество последователей. Я же считаю, что это [возмутительно и вопиюще нелепо][07].
ВозможноН, это и не так очевидно, но у Grunt есть множество плюсов по отношению
к Gulp, хотя и у Gulp есть плюсы по отношению к Grunt.

В этой статье я собираюсь **объяснить что из себя представляет Gulp**, так
как это достаточно новый инструмент, который появился около 6-ти месяцев назад.
Затем, _я хочу сравнить его с Grunt_, и показать, что какой из инструментов 
...
Возможно, [прочитав README][2], вы захотите пропустить эту часть, хотя, я бы
порекомендовал вам посмотреть еще и в исходный код Gulp — там мало сюрпризов,
скорее просто хорошо продуманный и краткий код.
...
Перед тем, как мы посмотрим на простой gulpfile.js, давайте посмотрим, что
представляет собой API Gulp. API содержит 5 методов, и, вобщем-то, это все, что
вам понадобиться. Это потрясающий лаконичный API, который действительно помогает
Gulp фокусироваться на главном — обработке задач сборки.
